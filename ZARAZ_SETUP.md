...
3. Add these three purposes:
   - **Purpose Name**: `analytics`
     ...
     - **Note**: Cloudflare will auto-generate a Purpose ID (e.g., "HYWH")
   - **Purpose Name**: `marketing`
     - **Description**: Marketing, advertising, and promotional content
     - **Note**: Cloudflare will auto-generate a Purpose ID (e.g., "Hhik")
   - **Purpose Name**: `functional`
     - **Description**: Essential functionality (always granted)
     - **Note**: Cloudflare will auto-generate a Purpose ID (e.g., "XwpD")

4. **IMPORTANT**: After creating purposes, note the auto-generated IDs shown in the "ID" column
   - These IDs are used in the code (see ConsentBanner.tsx)
   - Current IDs for this site:
     - `HYWH` = analytics
     - `Hhik` = marketing
     - `XwpD` = functional
...
The site uses this consent structure with Cloudflare-assigned purpose IDs:
```typescript
window.zaraz.consent.set({
  HYWH: boolean,   // analytics - GA4
  Hhik: boolean,   // marketing - Klaviyo, Trustpilot, Facebook Pixel
  XwpD: true,      // functional - Always granted
});
```

**Note**: The IDs (HYWH, Hhik, XwpD) are auto-generated by Cloudflare and must match the IDs shown in your Zaraz dashboard Settings → Privacy.

...
// Map product handles to ingredient page slugs
// All products containing Expedition Spiced Rum link to the same ingredients page
function getIngredientsSlug(handle: string): string {
  const expeditionSpicedProducts = [
  ...
  if (expeditionSpicedProducts.includes(handle)) {
    return 'expedition-spiced-rum'
  }

  // Fallback: strip brand prefix for any other products
                ...
                {isSpirit ? (
                  ...
                  <>
                    ...
                    <li className="flex items-start gap-3">
                      ...
                      <span>Distilled at our British partner distillery - proper craft, not factory production</span>
                    </li>
                    <li className="flex items-start gap-3">
                      <svg className="w-5 h-5 text-gold-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                      </svg>
                      <span>700 numbered bottles for general release per batch. Every one numbered. No reprints.</span>
                    </li>
                    <li className="flex items-start gap-3">
                      <svg className="w-5 h-5 text-gold-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              ...
              {sanityProduct?.relatedCocktails && sanityProduct.relatedCocktails.length > 0 ? (
                <div className="grid sm:grid-cols-2 gap-3">
                  {sanityProduct.relatedCocktails.filter(c => c?.slug?.current).map((cocktail) => (
                    ...
                    >
                      <svg className="w-4 h-4 text-gold-400 shrink-0 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>

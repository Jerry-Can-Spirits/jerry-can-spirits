...
import { equipmentBySlugQuery, equipmentSitemapQuery } from '@/sanity/queries'
...
export async function generateStaticParams() {
  const equipment = await client.fetch<{ slug: { current: string } }[]>(equipmentSitemapQuery)
  return equipment.map(({ slug }) => ({ slug: slug.current }))
}
        ...
        <div className="mt-12 space-y-8">
            ...
            {equipment.relatedCocktails && equipment.relatedCocktails.length > 0 && (
              <div className="bg-linear-to-br from-parchment-200/10 to-parchment-400/5 backdrop-blur-sm rounded-xl p-6 border border-gold-500/20">
                ...
                <RelatedCocktailsList cocktails={equipment.relatedCocktails} />
              </div>
            )}

...
Claims about where or how the rum is produced (distillation location, water source, molasses, sourcing, the named or unnamed producer) carry legal exposure and must not be added, changed, or restored without explicit founder sign-off. The approved framing is British, small batches, Caribbean rum base, and "macerated by our British partner distillery" as the only production formulation (never "macerated at", never "made at", never "blended"). The standing ingredient formulation is "seven real spices, two natural sweeteners, and bourbon oak for maturation"; never a bare botanicals count, and no bottle count in brand-level copy (counts are batch attributes). Do not assert a Welsh location, a specific water source, or that we use molasses (the Caribbean rum base is fermented upstream, before our process begins, so it describes work we do not do). When a task touches this class of copy, apply the copy discipline: remove a claim only where deletion leaves a clean, truthful sentence, otherwise stop and report rather than invent replacement copy. Run the grep in `docs/PROVENANCE_CHECKLIST.md` before shipping any copy change.

Claims about commercial relationships are the same class of claim. **No affiliate programme is live**: no links, no programme, no relationship with Master of Malt or any retailer. The dormant `budgetLink`/`premiumLink` schema fields are retained for a decision not yet taken and are not evidence of one. Publish no affiliate disclosure until a programme exists — a disclosure of a relationship we do not have misstates our commercial position just as an undisclosed real one would.

...
The trade portal is a **transitional** surface — it retires when distributor sales begin, so its hardening is scoped to that horizon rather than gold-plated. A **six-digit minimum PIN** is the deliberate long-term floor (six, not higher, trades brute-force headroom against bar-tablet usability; the global velocity counter carries the enumeration defence). Its enforcement is staged behind a one-off reissue of existing accounts — a PIN's length is unrecoverable once hashed, so every live account is reissued a known ≥6-digit PIN before the login floor is raised — and lands as a follow-up commit, not this one. **Turnstile-after-N-failures** is a considered-and-deferred follow-up, not an oversight: the CSP is already Turnstile-ready and the friction would land only on someone already failing repeatedly, so it is defence-in-depth rather than urgent. The `pending/` upload R2 lifecycle expiry is already in place (see Data).

...
5. **Stale docs** — `docs/DEPENDENCIES_STATUS.md` (Nov 2025 header; internally contradictory on Pages vs Workers, and line ~150 still claims the CSP is set "via middleware and `_headers`") should be **retired**: it is superseded by this file, `README.md`, and `docs/CSP_AUDIT.md`, all current. `docs/SHOPIFY_SETUP.md` likewise has a stale "deploy to Cloudflare Pages" line. The `public/_headers` CSP-via-middleware comment and the `docs/CSP_AUDIT.md` nonce record were both corrected 2026-07-19 (audit finding #10).

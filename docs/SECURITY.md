...
1. **HTTP headers.** Every response carries HSTS (with preload), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, a Permissions-Policy denying what the app does not use, and a CSP. Framing is denied by default via `frame-ancestors 'none'` in the CSP, which supersedes and replaces `X-Frame-Options` (the site does not set `X-Frame-Options`), with narrow, documented per-route exceptions.
...
### Headers and CSP
The single source of truth is `next.config.ts` (`buildCsp()`, lines ~13–152); `middleware.ts` deliberately sets no security headers (it handles bot detection and geo cookies only). The CSP uses `'unsafe-inline'` with an individually-allowlisted set of third-party hosts, not nonces or hashes. Whether a nonce or hash CSP can work on this stack is under re-investigation, not settled: the earlier conclusion that nonces cannot propagate here was reached while `middleware.ts` was silently never executing (a root-level file in a `src/`-rooted project, since relocated to `src/middleware.ts`). See `docs/CSP_AUDIT.md`. Full third-party allowlist (Cookiebot, GTM/GA, Klaviyo, Sentry, Trustpilot, Facebook, Google Ads/AdSense, Metricool, Cloudflare, Ahrefs, Turnstile); `script-src` accepts `'unsafe-inline'` (documented acceptance); `'unsafe-eval'` dev-only. Per-route exceptions: `/qr/*` may be framed by `info.qr.jerrycanspirits.co.uk`; `/api/pouriq/invoices/*` allows `frame-ancestors 'self'`. HSTS preload, Permissions-Policy, and COOP are all set. `public/_headers` is a backup for static files only and does not carry the CSP.
...
### Dependencies
Dependabot (npm weekly + github-actions, grouped, reviewers set). CI runs lint + typecheck and a secrets-injected build with a summary gate, then purges Cloudflare cache post-push. No audit/SAST step. Node is pinned to 24.12.0 in `.nvmrc`, `.node-version` and the `package.json` engines floor, all three in agreement.
...
3. ~~**Resolve Node pinning**~~ — closed. `.nvmrc`, `.node-version` and the engines floor all read 24.12.0.
...
5. **Stale docs** — `docs/DEPENDENCIES_STATUS.md` (Nov 2025 header; internally contradictory on Pages vs Workers, and line ~150 still claims the CSP is set "via middleware and `_headers`") should be **retired**: it is superseded by this file, `README.md`, and `docs/CSP_AUDIT.md`, all current. `docs/SHOPIFY_SETUP.md` had the same "deploy to Cloudflare Pages" line, corrected 2026-09-24 along with its collection handles and a note marking the pre-launch steps as a record. The `public/_headers` CSP-via-middleware comment and the `docs/CSP_AUDIT.md` nonce record were both corrected 2026-07-19 (audit finding #10). Still outstanding in this class: `docs/COMPONENT_USAGE_EXAMPLES.md` documents two components (`InstagramFeed`, `TrustpilotWidget`) that do not exist, while the three that do (`FAQAccordion`, `SectionHeading`, `ScrollRow`) are documented nowhere; `docs/BRAND_GUIDELINES.md` has five wrong colour hexes and predates the light/dark band system; `docs/SHOPIFY_INTEGRATION_PLAN.md` describes a pre-launch shop.

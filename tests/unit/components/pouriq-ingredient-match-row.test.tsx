// @vitest-environment jsdom
import { render, screen, fireEvent, cleanup } from '@testing-library/react'
import '@testing-library/jest-dom/vitest'
import { afterEach, test, expect, vi } from 'vitest'
import { IngredientMatchRow } from '@/components/pouriq/IngredientMatchRow'
import type { IngredientType } from '@/lib/pouriq/types'

afterEach(cleanup)

const baseProps = {
  extractedName: "Gordon's Gin",
  rawMeasurement: '50ml',
  inferredType: 'spirit' as IngredientType,
  matchKind: 'no-match' as const,
  libraryEntries: [],
  serveUnits: {},
  state: { pour_ml: 50, unit_count: null, recipe_unit: null, recipe_qty: null },
  onChange: () => {},
}

test('no-match row can pick an existing library entry', () => {
  const onChange = vi.fn()
  render(<IngredientMatchRow {...baseProps} matchKind="no-match"
    libraryEntries={[{ id: '9', name: "Gordon's London Dry", ingredient_type: 'spirit' } as any]}
    state={{ pour_ml: 50, unit_count: null, recipe_unit: null, recipe_qty: null }}
    onChange={onChange} />)
  fireEvent.focus(screen.getByRole('textbox'))
  fireEvent.change(screen.getByRole('textbox'), { target: { value: 'gor' } })
  fireEvent.click(screen.getByText(/Gordon's London Dry/))
  expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ existing_library_id: '9' }))
})

const stagedState = {
  pour_ml: 50,
  unit_count: null,
  recipe_unit: null,
  recipe_qty: null,
  new_library: {
    name: "Gordon's Gin",
    ingredient_type: 'spirit' as IngredientType,
    base_unit: 'ml' as const,
    pack_size: 700,
    price_p: null,
    price_includes_vat: false,
    purchase_qty: 1,
  },
}

test('staging form VAT toggle defaults to Ex and stores net basis on Inc', () => {
  const onChange = vi.fn()
  render(<IngredientMatchRow {...baseProps} state={stagedState} onChange={onChange} />)

  const incBtn = screen.getByRole('button', { name: /inc vat/i })
  const exBtn = screen.getByRole('button', { name: /ex vat/i })

  // Default is Ex (net): Ex is pressed, Inc is not.
  expect(exBtn).toHaveAttribute('aria-pressed', 'true')
  expect(incBtn).toHaveAttribute('aria-pressed', 'false')

  // Clicking Inc stages the gross basis.
  fireEvent.click(incBtn)
  expect(onChange).toHaveBeenCalledWith(
    expect.objectContaining({
      new_library: expect.objectContaining({ price_includes_vat: true }),
    }),
  )
})

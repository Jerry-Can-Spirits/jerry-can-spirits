/**
 * The homepage grid's selection rule.
 *
 * The grid promises a first visit sees everything purchasable with a price,
 * including something cheap enough for a first order. So the rule is pinned:
 * accessories and the tree fund never appear, nothing unavailable appears,
 * spirits lead cheapest first, then the kit cheapest first, capped at the
 * grid size.
 */
import { describe, expect, it } from 'vitest'
import { HOMEPAGE_GRID_LIMIT, selectHomepageProducts } from '@/lib/homepage-products'
import type { ShopifyProduct } from '@/lib/shopify'

function p(handle: string, amount: string, productType: string, available = true): ShopifyProduct {
  return {
    id: `gid://shopify/Product/${handle}`,
    title: handle,
    handle,
    description: '',
    productType,
    priceRange: { minVariantPrice: { amount, currencyCode: 'GBP' }, maxVariantPrice: { amount, currencyCode: 'GBP' } },
    images: [],
    variants: [{ id: `v-${handle}`, title: 'Default Title', price: { amount, currencyCode: 'GBP' }, availableForSale: available }],
  }
}

const catalogue = [
  p('metal-logo-keyring', '7.50', 'Barware'),
  p('jerry-can-spirits-premium-gift-pack', '90.00', 'Spirits'),
  p('uk-tree-fund', '1.00', ''),
  p('stainless-steel-jigger', '12.50', 'Barware'),
  p('jerry-can-spirits-expedition-spiced-rum', '40.00', 'Spirits'),
  p('jerry-can-spirits-expedition-spiced-rum-presentation-box', '1.99', ''),
  p('jerry-can-spirits-expedition-pack-spiced-rum-6-bottles', '228.00', 'Spirits'),
  p('bar-blade-bottle-opener', '7.50', 'Barware'),
  p('sold-out-thing', '15.00', 'Barware', false),
]

describe('selectHomepageProducts', () => {
  it('leads with the spirits cheapest first, then the kit cheapest first', () => {
    expect(selectHomepageProducts(catalogue).map((x) => x.handle)).toEqual([
      'jerry-can-spirits-expedition-spiced-rum',
      'jerry-can-spirits-premium-gift-pack',
      'jerry-can-spirits-expedition-pack-spiced-rum-6-bottles',
      'metal-logo-keyring',
      'bar-blade-bottle-opener',
      'stainless-steel-jigger',
    ])
  })

  it('never shows the tree fund, the presentation box or anything not for sale', () => {
    const handles = selectHomepageProducts(catalogue).map((x) => x.handle)
    expect(handles).not.toContain('uk-tree-fund')
    expect(handles).not.toContain('jerry-can-spirits-expedition-spiced-rum-presentation-box')
    expect(handles).not.toContain('sold-out-thing')
  })

  it('keeps the first row cheap: something under £10 is within the first six', () => {
    const first = selectHomepageProducts(catalogue).slice(0, 6)
    expect(first.some((x) => parseFloat(x.priceRange.minVariantPrice.amount) < 10)).toBe(true)
  })

  it('caps at the grid size', () => {
    const many = Array.from({ length: 30 }, (_, i) => p(`item-${i}`, String(10 + i), 'Barware'))
    expect(selectHomepageProducts(many)).toHaveLength(HOMEPAGE_GRID_LIMIT)
    expect(selectHomepageProducts(many, 5)).toHaveLength(5)
  })

  it('treats a product with no variants and no availability flag as for sale', () => {
    const bare: ShopifyProduct = { ...p('bare', '9.00', 'Barware'), variants: undefined }
    expect(selectHomepageProducts([bare]).map((x) => x.handle)).toEqual(['bare'])
  })
})
